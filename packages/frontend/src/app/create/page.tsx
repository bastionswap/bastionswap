          ...
          {poolStep === "done" && hash && (
            <div className="mt-4 rounded-xl bg-emerald-50 border border-emerald-200 p-4 text-center">
              ...
              <button
                ...
                className="mt-1 inline-flex items-center gap-1.5 text-xs text-emerald-600/70 font-mono hover:text-emerald-700 transition-colors"
                title="Copy transaction hash"
              >
                {hash.slice(0, 10)}...{hash.slice(-8)}
                {copiedHash ? (
                  <span className="shrink-0 font-sans text-emerald-600">Copied!</span>
                ) : (
                  <svg className="h-3.5 w-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
